...
Four public NHL endpoints, what each returns, and the quirks that matter. All
were live as of July 2026. They are **undocumented** — treat availability and
shape as things to verify, not guarantee.
...
- Every other entity carries `playerId`, `x`, `y`, `sweaterNumber`, `teamId`,
  `teamAbbrev`.
- Coordinates are **inches from the corner**; see [METHODS §3](METHODS.md) for
  the transform and the y-axis warning.

**Availability floor is 2023-24.** A six-season probe found a hard, clean
boundary: 2020-21, 2021-22 and 2022-23 return 403 for *every* sprite, while
2023-24, 2024-25 and 2025-26 return 200 and parse. League-wide player tracking
became operational in 2021-22, but the public goal-recreation feed is a separate
pipeline from the tracking system itself. **The usable archive is three
seasons.**
...
| Null puck | ~1% of goals have no puck coordinates at the final frame; the pipeline walks back to a valid frame. One goal in 2023-24 had no valid puck in any frame and is flagged |
| Missing sprites | A goal with no public recreation returns 403. This is data, not failure — mostly shootout deciders |

**Size:** ~24,000 files, ~6.8 GB across three seasons.
...
for, the `rosterSpots` that resolve every player id, and — critically — an
**independent record of where each goal was scored from**, which is what makes
the shot-frame validation in [METHODS §10](METHODS.md) possible.
...
| `plays[].homeTeamDefendingSide` | `"left"` / `"right"`. Fixes attacking direction, and therefore the target net, with no dependence on tracking |
| `details.scoringPlayerId` | **The anchor for shot detection** ([METHODS §7.2](METHODS.md)) |
| `details.xCoord`, `yCoord` | The scorer's own record of the shot location — the independent validation target |
...
The classic HTML reports carry the shift data for the games the JSON feed serves
empty. Parsed with BeautifulSoup into rows matching the JSON schema exactly, so
both sources feed one table with a `source` column recording which.
...
**Validation:** recovered TOI correlates **0.99996** with the JSON feed's on
games where both exist. 1,010 files fetched for 2025-26 with zero errors.
...
This is a **separate data source, not an input to the goal tracking.** It runs
as an isolated step so a wholesale failure never touches the core archive. Its
one notable advantage: **Edge coverage starts in 2021-22**, two seasons before
the sprite floor.
